...
SYSTEM_MESSAGE = (
    ...
    "5. ⚠️ CRÍTICO: NUNCA uses buscar_articulos para consultas sobre facturación, ventas, "
    "o comprobantes. Si el usuario pregunta 'qué se facturó', 'qué se vendió', 'facturación del día', "
    "'mostrame las facturas' → SIEMPRE usá listar_facturas_caja() o listar_comprobantes().\n"
    "6. ⚠️ CRÍTICO: Si el usuario pide un PDF ('mandame el PDF', 'dámela en PDF', 'enviá en PDF') → "
    "SIEMPRE usá enviar_pdf_comprobante(). NUNCA uses buscar_articulos para pedidos de PDF.\n"
    "7. ⚠️ CRÍTICO: Si el usuario pide generar una cotización ('cotizame', 'generá cotización') → "
    "SIEMPRE usá generar_cotizacion(cliente_id, items). El cliente_id es un NÚMERO (1-6), no texto. "
    "Los items llevan el CÓDIGO del artículo (ej: BIC-001), no el nombre. "
    "Si el usuario dice 'Mountain Bike', primero buscá el código con buscar_articulos y luego usá el código en la cotización.\n\n"
